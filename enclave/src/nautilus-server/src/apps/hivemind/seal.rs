...
lazy_static::lazy_static! {
    ...
    /// Set in enclave here when /provision_delegate_key is called. This is the enclave delegate key
    /// used by recall() to sign MemWal requests.
    pub static ref ENCLAVE_DELEGATE_KEY: Arc<RwLock<Option<String>>> = Arc::new(RwLock::new(None));
}
...
pub async fn init_seal_key_load(
    ...
    if ENCLAVE_DELEGATE_KEY.read().await.is_some() {
...
/// This endpoint decrypts the enclave delegate key using cached keys from the complete_key_load phase.
...
/// specific endpoint.
pub async fn provision_delegate_key(
    State(_state): State<Arc<AppState>>,
    Json(request): Json<ProvisionDelegateKeyRequest>,
) -> Result<Json<ProvisionDelegateKeyResponse>, EnclaveError> {
    ...
    .map_err(|e| EnclaveError::GenericError(format!("Failed to decrypt enclave delegate key: {e}")))?;

    // Convert decrypted bytes to UTF-8 string.
    let api_key_str = String::from_utf8(api_key_bytes)
        .map_err(|e| EnclaveError::GenericError(format!("Invalid UTF-8 in delegate key: {e}")))?;

    // Store the delegate key for recall() to use.
    let mut api_key_guard = (*ENCLAVE_DELEGATE_KEY).write().await;
    *api_key_guard = Some(api_key_str);

    Ok(Json(ProvisionDelegateKeyResponse {
        status: "OK".to_string(),
    }))
}
...
/// The Seal-decrypted enclave delegate key (hex), if the bootstrap has run.
/// recall() prefers this over the env fallback, so in production the key is only
/// ever present in enclave memory after a successful attestation-gated Seal load.
pub async fn provisioned_delegate_key() -> Option<String> {
    ENCLAVE_DELEGATE_KEY.read().await.clone()
}

/// Simple ping for host-only access.
async fn ping() -> &'static str {
    "Pong!"
}

/// Spawn a host-only bootstrap server on localhost:3001. The HOST (which has
/// internet) drives the Seal two-phase load by calling these admin endpoints and
/// relaying FetchKeyRequests to the key servers; the enclave never reaches the
/// network itself. Decryption only succeeds if this enclave is the registered,
/// attested one (enforced by the seal_approve policy).
pub async fn spawn_host_init_server(state: Arc<AppState>) -> Result<(), EnclaveError> {
    let host_app = Router::new()
        ...
        .route("/admin/complete_seal_key_load", post(complete_seal_key_load))
        .route("/admin/provision_delegate_key", post(provision_delegate_key))
        .with_state(state);
    ...
    info!("HiveMind host-only init server listening on {}", host_listener.local_addr().unwrap());
